# Allows to embed the chart into a tkinter canvas
import tkinter as tk
from matplotlib.backends.backend_tkagg import (FigureCanvasTkAgg)

import matplotlib.pyplot as plt
from data import sales_data, inventory_data,product_data,sales_year_data,inventory_month_data

plt.rcParams["axes.prop_cycle"] = plt.cycler(
    color=["#03A9F4", "#29B6F6", "#4FC3F7", "#0288D1", "#B3E5FC"])
# Chart 1: Bar chart of sales data
fig1, ax1 = plt.subplots()# .subplots() generates one or more plots in the same figure.
ax1.bar(sales_data.keys(), sales_data.values())#The 'keys' and 'values' are taken from the dictionary list.

# Set chart title and label
ax1.set_title("Sales by product")
ax1.set_xlabel("Product")
ax1.set_ylabel("sales")
#plt.show()#shows the code on a viewer

# Chart 2: Horizontal bar chart of inventory data
fig2, ax2 = plt.subplots()
ax2.barh(list(inventory_data.keys()), inventory_data.values())#.barh() creates a horizontal bar plot and list() function converts an iterable (e.g string, tuple or dictionary) into a list.

# Set chart title and label
ax2.set_title("Inventory by Product")
ax2.set_xlabel("Inventory")
ax2.set_ylabel("Product")
#plt.show()

# Chart 3: Pie chart chart of product data
fig3, ax3 = plt.subplots()
ax3.pie(product_data.values(), labels=product_data.keys(), autopct='%1.1f%%')
ax3.set_title("Product \nBreakdown")
#plt.show()

# Chart 4: Line chart of sales by year
fig4, ax4 = plt.subplots()
ax4.plot(list(sales_year_data.keys()), list(sales_year_data.values()))
ax4.set_title("Sales by Year")
ax4.set_xlabel("Year")
ax4.set_ylabel("Sales")
#plt.show()

# Chart 5: Area chart of inventory by month
fig5, ax5 = plt.subplots()
ax5.fill_between(inventory_month_data.keys(), inventory_month_data.values())
ax2.set_title("Inventory by Month")
ax2.set_xlabel("Month")
ax2.set_ylabel("Inventory")
#plt.show()

# Create a window and add charts
root = tk.Tk()
root.title("Dashboard")
root.state('zoomed')# maximised to take up entire sceen space

# Create a side frame
side_frame = tk.Frame(root, bg="#039BE5")
side_frame.pack(side="left", fill="y")

# Create a label
label = tk.Label(side_frame, text="Dashboard", bg="#039BE5", fg="#FFF", font=25)
label.pack(pady=50, padx=20)

# Join frames together
charts_frame = tk.Frame(root)
charts_frame.pack()


# Create upper frame
upper_frame = tk.Frame(charts_frame)
upper_frame.pack(fill="both", expand=True) #Geometry manager to position frames, fill X & Y axis and then expands the frame

canvas1 = FigureCanvasTkAgg(fig1, upper_frame)
canvas1.draw()# .draw() function shows the canvas
canvas1.get_tk_widget().pack(side="left", fill="both", expand=True)#Tkinter widget then call .pack() to place chart on the left side

canvas2 = FigureCanvasTkAgg(fig2, upper_frame)
canvas2.draw()
canvas2.get_tk_widget().pack(side="left", fill="both", expand=True)

canvas3 = FigureCanvasTkAgg(fig3, upper_frame)
canvas3.draw()
canvas3.get_tk_widget().pack(side="left", fill="both", expand=True)

# Create lower frame
lower_frame = tk.Frame(charts_frame)
lower_frame.pack(fill="both", expand=True)

canvas4 = FigureCanvasTkAgg(fig4, lower_frame)
canvas4.draw()
canvas4.get_tk_widget().pack(side="left", fill="both", expand=True)

canvas5 = FigureCanvasTkAgg(fig5, lower_frame)
canvas5.draw()
canvas5.get_tk_widget().pack(side="left", fill="both", expand=True)

root.mainloop() #Inifite loop running 